...
const GAMES = [
  { name: "Coinflip",  wagered: "54,200", winRate: 0.61 },
  { name: "Jackpot",   wagered: "38,100", winRate: 0.44 },
  { name: "Upgrader",  wagered: "19,800", winRate: 0.52 },
  { name: "Dice",      wagered: "12,400", winRate: 0.38 },
];

const HISTORY = [
  { time: "2h ago",  game: "Coinflip", result: "won",  amount: "+2,400" },
  { time: "3h ago",  game: "Jackpot",  result: "lost", amount: "-1,800" },
  { time: "5h ago",  game: "Upgrader", result: "won",  amount: "+4,100" },
  { time: "8h ago",  game: "Dice",     result: "lost", amount: "-600"   },
  { time: "12h ago", game: "Coinflip", result: "won",  amount: "+950"   },
];
      ...
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "24px", marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
            {/* Avatar */}
            <div style={{ width: 72, height: 72, borderRadius: "50%", background: "#222", border: "2px solid rgba(255,255,255,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, fontWeight: 700, color: "#fff", flexShrink: 0 }}>D</div>
            
            {/* Info */}
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                <span style={{ fontSize: 20, fontWeight: 700 }}>DragonSlayer99</span>
                <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, background: "#1e1e1e", color: "#888", border: "1px solid rgba(255,255,255,0.08)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Member</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#888" }}>
                <MessageCircle size={13} />
                DragonSlayer#0001
              </div>
              <div style={{ fontSize: 12, color: "#444", marginTop: 4 }}>ID: 7f3a92b1</div>
            </div>
        ...
        {/* ── Stats Grid ── */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 0, border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, overflow: "hidden", marginBottom: 24 }}>
          {[
            { label: "WAGERED", value: "124,500", color: "#fff" },
            { label: "WON",     value: "89,200",  color: "#4ade80" },
            { label: "LOST",    value: "35,300",  color: "#f87171" },
          ].map((s, i) => (
            <div key={i} style={{
              padding: "22px 20px",
              background: "#111",
              ...
              <div style={{ fontSize: 11, color: "#444", marginTop: 3 }}>gems</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, overflow: "hidden", marginBottom: 24 }}>
          ...
          {GAMES.map((g, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 20px", borderBottom: i < GAMES.length-1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
              ...
              <div style={{ fontSize: 13, color: "#888", fontVariantNumeric: "tabular-nums", minWidth: 72 }}>{g.wagered}</div>
              {/* Win rate bar */}
        ...
        <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, overflow: "hidden", marginBottom: 32 }}>
          ...
          {HISTORY.map((h, i) => (
            <div key={i} style={{
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: h.result === "won" ? "#4ade80" : "#f87171", minWidth: 60, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{h.amount}</div>
            </div>
          ))}
        </div>

        {/* ── Footer ── */}
        <div style={{ textAlign: "center", fontSize: 12, color: "#333" }}>Member since July 2025</div>
