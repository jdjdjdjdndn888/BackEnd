...
exports.getUserInventory = asyncHandler(async (req, res) => {
  ...
  const userId = parseRobloxUserId(req.params.userid);
  if (userId === null) return res.status(400).json({ message: "userid must be a positive Roblox user ID." });

  const user = await users.findOne({ userid: userId }).lean();
  if (!user) return res.status(404).json({ message: "User not found." });

  const inv = await inventorys.find({ owner: userId }).lean();
...
exports.deleteUserInventoryItems = asyncHandler(async (req, res) => {
  ...
  const userId = parseRobloxUserId(req.body?.userid);
  if (userId === null) return res.status(400).json({ message: "userid must be a positive Roblox user ID." });

  const inventoryIds = normalizeInventoryIds(req.body?.inventoryIds);
  if (!inventoryIds) {
    return res.status(400).json({ message: "No items selected." });
  }
  const result = await inventorys.deleteMany({ _id: { $in: inventoryIds }, owner: userId });
  logAction(req.adminUser, "Delete User Inventory Items", String(userId), `Deleted ${result.deletedCount} item(s)`);
  res.json({ success: true, message: `Deleted ${result.deletedCount} item(s).` });
});

// ── Repair corrupted inventory items (NaN / null itemids from bad bot inserts) ─
exports.repairInventory = asyncHandler(async (req, res) => {
  if (!req.adminUser || !OWNER_TIER.includes(req.adminUser.rank)) {
    return res.status(403).json({ message: "Owner tier only." });
  }
  const userId = parseRobloxUserId(req.body?.userid);
  if (userId === null) {
    return res.status(400).json({ message: "userid must be a positive Roblox user ID." });
  }

  const user = await users.findOne({ userid: userId }).select("userid username").lean();
  if (!user) return res.status(404).json({ message: "User not found." });

  // Find entries whose itemid is NaN, null, 0, or not a positive safe integer.
  // NaN in MongoDB is stored as a special BSON double — $type 1 covers all doubles including NaN.
  // We match: itemid IS NaN (isNaN filter via aggregation), or itemid <= 0, or itemid is null.
  const filter = {
    owner: userId,
    $expr: {
      $or: [
        { $eq: ["$itemid", null] },
        { $lte: ["$itemid", 0] },
        { $not: { $eq: ["$itemid", { $round: ["$itemid", 0] }] } },
      ],
    },
  };

  const corrupt = await inventorys.find(filter).lean();
  const count = corrupt.length;

  if (count === 0) {
    return res.json({ success: true, message: "No corrupted inventory entries found.", deleted: 0 });
  }

  const ids = corrupt.map(e => e._id);
  await inventorys.deleteMany({ _id: { $in: ids } });

  logAction(req.adminUser, "Repair Inventory", `${user.username} (${userId})`, `Deleted ${count} corrupted item(s) with invalid itemid`);
  return res.json({ success: true, message: `Deleted ${count} corrupted inventory entry/entries with invalid itemid.`, deleted: count });
});
...
exports.transferInventoryItems = asyncHandler(async (req, res) => {
  ...
  const fromId = parseRobloxUserId(fromUserId);
  const toId = parseRobloxUserId(toUserId);
  if (fromId === null || toId === null) {
    return res.status(400).json({ message: "fromUserId and toUserId must be positive Roblox user IDs." });
  }
  const normalizedInventoryIds = normalizeInventoryIds(inventoryIds);
  if (!normalizedInventoryIds) {
    return res.status(400).json({ message: "No items selected." });
  }

  const [fromUser, toUser] = await Promise.all([
    users.findOne({ userid: fromId }).lean(),
    users.findOne({ userid: toId }).lean(),
  ]);
  ...
  const result = await inventorys.updateMany(
    { _id: { $in: normalizedInventoryIds }, owner: fromUser.userid },
...
exports.scrapeItems = asyncHandler(async (req, res) => {
  ...
  if (manualItems && Array.isArray(manualItems) && manualItems.length > 0) {
    ...
    scraped = withImages.map((it) => ({
      ...
      itemimage: `${process.env.PUBLIC_API_URL || "https://api.gemtide.win"}/item-image?name=${encodeURIComponent(it.itemname)}`,
      game: it.game,
    }));
  }
