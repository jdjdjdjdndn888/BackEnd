...
exports.me = asyncHandler(async (req, res) => {
  try {
    ...
    const [user, inventory, userhistory] = await Promise.all([
      users.findOne({ userid: numUserId }).lean(),
      inventorys.find({ owner: numUserId }).lean(),
      history.find({ userid: numUserId }).lean(),
    ]);
...
exports.profile = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const query = userid ? { userid } : { username: { $regex: new RegExp(`^${escaped}$`, "i") } };
  ...
  if (userCode) {
    ...
    try {
      ...
      if (!dbUser) {
        ...
        // Use updateOne to avoid Mongoose full-document validation on save,
        // which would fail if any required field was corrupted by external writes.
        await users.updateOne(
          { userid: userId },
          { $set: { thumbnail: userThumbnail[0].imageUrl } }
        );
      }
...
exports.inventory = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Fast path: $match uses the { owner: 1 } index added to the schema.
    // $lookup uses localField/foreignField so MongoDB can hit the unique
    // index on items.itemid instead of scanning with $expr.
...
exports.withdraw = asyncHandler(async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const user = await users.findOne({ userid: numUserId }).session(session);
      if (!user) {
      ...
      inventoryItems.forEach(inventoryItem => {
        ...
        withdrawalsToInsert.push({
          ...
          game: dbItem.game || null, // game is optional in withdraws schema
...
exports.linkdiscord = asyncHandler(async (req, res) => {
  ...
  if (!clientsecret) {
    console.error("[linkdiscord] DISCORD_CLIENT_SECRET env var is not set on this server.");
    return res.status(500).json({ message: "Discord linking is not configured on this server — DISCORD_CLIENT_SECRET env var is missing. Ask the site owner to set it." });
  }
  ...
  try {
    ...
    if (discordErr?.error === "invalid_client") {
      console.error("[linkdiscord] invalid_client — check DISCORD_CLIENT_SECRET and that redirect URI matches Discord app settings. redirect_uri used:", uri);
      return res.status(500).json({ message: "Discord app credentials are wrong on this server — DISCORD_CLIENT_SECRET may be incorrect or the redirect URI is not registered in the Discord Developer Portal." });
    }
