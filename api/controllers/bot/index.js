...
const withdrawLog = require("../../modules/withdrawlog.js");
...
exports.Deposit = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({
    message: "Deposit process completed",
    depositResults,
    totalValue,
    discordId: user.discordid ? String(user.discordid) : null,
    discordUsername: user.discordusername || null,
    avatar: user.thumbnail || null,
    totalDeposited: (user.deposited || 0) + totalValue,
    totalWithdrawn: user.withdrawn || 0,
  });
});
...
exports.confirmWithdrawAll = asyncHandler(async (req, res) => {
  ...
  // ── Idempotency check ────────────────────────────────────────────────────
  // If the bot sends a stable withdrawId, record it atomically before
  // deleting withdraw records. A duplicate-key error means this exact
  // payload was already processed — return success so the bot doesn't retry.
  const withdrawId = req.body.withdrawId || req.headers["x-withdraw-id"];
  if (withdrawId) {
    try {
      await withdrawLog.create({ withdrawId: String(withdrawId), userid: numUserId });
    } catch (idempErr) {
      if (idempErr.code === 11000) {
        return res.status(200).json({ message: "OK", note: "Already processed (idempotency key matched)" });
      }
      throw idempErr;
    }
  }
  ...
  try {
    ...
    // Compute the R-value of confirmed items so we can track cumulative withdrawn
    let totalWithdrawnValue = 0;
    if (idsToDelete.length > 0) {
      const matchedWithdraws = userWithdraws.filter((w) =>
        idsToDelete.some((id) => id.toString() === w._id.toString())
      );
      const uniqueItemNames = [...new Set(matchedWithdraws.map((w) => w.itemname).filter(Boolean))];
      if (uniqueItemNames.length > 0) {
        const itemDocs = await items.find({ itemname: { $in: uniqueItemNames } });
        const itemValueMap = {};
        itemDocs.forEach((doc) => { itemValueMap[(doc.itemname || "").toLowerCase()] = doc.itemvalue || 0; });
        totalWithdrawnValue = matchedWithdraws.reduce(
          (sum, w) => sum + (itemValueMap[(w.itemname || "").toLowerCase()] || 0),
          0
        );
      }
    }

    const newTotalWithdrawn = (user.withdrawn || 0) + totalWithdrawnValue;
    ...
    if (totalWithdrawnValue > 0) {
      users.updateOne({ userid: numUserId }, { $inc: { withdrawn: totalWithdrawnValue } }).catch((err) => {
        console.error("[confirmWithdrawAll] failed to update withdrawn total:", err.message);
      });
    }
  } catch (error) {
...
/**
 * POST /bots/heartbeat
 * Called by the Lua bot every 60 s while in-game, and once on shutdown.
 * Body: { botUserId, game, inGame, gems, hugeCount, inventory: [{name,count}] }
 */
exports.botHeartbeat = asyncHandler(async (req, res) => {
  const { game, inGame, gems, hugeCount, inventory } = req.body;
  const botUserId = Number(req.body.botUserId);
  if (!botUserId || !game) return res.status(400).json({ message: "botUserId and game required" });

  const gameRegex = new RegExp("^" + escapeRegex(game) + "$", "i");
  const bot = await botss.findOne({ userid: botUserId, game: { $regex: gameRegex } });
  ...
  const update = {
    ...
    lastSeen: now,
    gems:     Number(gems)      || 0,
    hugeCount: Number(hugeCount) || 0,
    inventorySnapshot: Array.isArray(inventory) ? inventory.slice(0, 200) : [],
  };

  if (!inGame) update.lastLeftAt = now;

  await botss.updateOne({ _id: bot._id }, { $set: update });

  return res.status(200).json({ message: "OK" });
});

/**
 * GET /bots/:game/inventory/:botId
 * Returns detailed inventory snapshot for a specific bot.
 * Requires user auth (public-facing).
 */
exports.getBotInventory = asyncHandler(async (req, res) => {
  const { game, botId } = req.params;
  if (!game || !botId) return res.status(400).json({ message: "Invalid params" });

  const gameRegex = new RegExp("^" + escapeRegex(game) + "$", "i");
  const bot = await botss.findOne(
    { _id: botId, game: { $regex: gameRegex } },
    { name: 1, inGame: 1, lastSeen: 1, lastLeftAt: 1, gems: 1, hugeCount: 1, inventorySnapshot: 1 }
  ).lean();
