...
// ── Auto-stand timer (60s inactivity) ────────────────────────────────────────
// If the active player doesn't hit or stand within 60 seconds, the server
// automatically stands them and resolves the round normally.
const turnTimers = new Map();      // gameId (string) → NodeJS.Timeout
const autoStandRunning = new Set(); // gameId (string) → lock guard

function clearTurnTimer(gameId) {
  const key = String(gameId);
  const t = turnTimers.get(key);
  if (t) { clearTimeout(t); turnTimers.delete(key); }
}

function startTurnTimer(gameId, io) {
  clearTurnTimer(gameId);
  const key = String(gameId);
  const t = setTimeout(() => {
    turnTimers.delete(key);
    autoStandGame(key, io).catch((err) => console.error("auto-stand error:", err));
  }, 60 * 1000);
  turnTimers.set(key, t);
}

async function autoStandGame(gameId, io) {
  if (autoStandRunning.has(gameId)) return;
  autoStandRunning.add(gameId);
  const session = await mongoose.startSession();
  let finalUpdate, resolved, payout;
  try {
    await session.withTransaction(async () => {
      const game = await blackjack.findOne({ _id: gameId, active: true, turn: "player" }).session(session);
      if (!game) return;

      const playerKey = game.PlayerOne.isDealer ? "PlayerTwo" : "PlayerOne";
      const player = game[playerKey];
      if (!player || player.busted || player.stood) return;

      // Auto-stand the non-dealer player
      player.stood = true;
    ...
    if (finalUpdate && resolved) {
      // Re-use finishSideEffects with a shim so it can call req.app.get("io")
      const fakeReq = { app: { get: () => io } };
      await finishSideEffects(finalUpdate, payout, fakeReq);
    }
  } catch (err) {
    console.error("autoStandGame:", err);
  } finally {
    autoStandRunning.delete(gameId);
    session.endSession();
  }
}
  ...
  try {
    ...
    try {
      ...
      if (resolved) {
        ...
        // Game is live and waiting for the non-dealer to act.
        startTurnTimer(finalUpdate._id, req.app.get("io"));
      }
...
async function actOnGame(req, res, action) {
  // Clear the inactivity timer as soon as we receive the player's action.
  const { gameid: earlyGameid } = req.body || {};
  if (earlyGameid) clearTurnTimer(earlyGameid);
  ...
  try {
    ...
    try {
      if (resolved) {
        ...
        req.app.get("io").emit("BLACKJACK_UPDATE", finalUpdate);
        // Player hit but didn't bust — restart the 60-second inactivity timer.
        startTurnTimer(finalUpdate._id, req.app.get("io"));
      }
    } catch (sideEffectError) {
