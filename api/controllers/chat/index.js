...
exports.sendchat = asyncHandler(async (req, res, next, io) => {
  ...
  if (user.level < 0) {
    return res.status(400).json({ "message": "You must be logged in to chat!" });
  }
  ...
  if (mentionMatch) {
    for (const mention of mentionMatch) {
      ...
      try {
        const mentionedUser = await users.findOne({ username: { $regex: new RegExp(`^${mentionedName}$`, "i") } }).select("userid");
        if (mentionedUser) {
          const mentionedSocketIds = userSockets.get(mentionedUser.userid) || [];
