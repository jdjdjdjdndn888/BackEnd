...
// Respond to CORS pre-flight OPTIONS requests before originGuard runs.
// Without this, complex requests (custom headers like Authorization, non-GET
// methods) send an OPTIONS preflight that falls through to routes, gets a 404,
// and the browser treats the CORS negotiation as failed.
...
// of duplicate files.
// Gem denomination names that must never go to the PS99 pet catalog.
// These items use a fixed Discord emoji image; redirect immediately.
const GEM_NAME_RE = /^(?:1m|5m|10m|25m|50m|100m)\s+gems?$/i;
const GEM_EMOJI_URL = "https://cdn.discordapp.com/emojis/1530173247193092146.webp?size=96";
...
app.get("/item-image", async (req, res) => {
  ...
  // Gems are not PS99 pets — redirect to the correct emoji before hitting the catalog.
  if (GEM_NAME_RE.test(itemName)) {
    res.setHeader("Cache-Control", "public, max-age=604800, immutable");
    res.setHeader("Cross-Origin-Resource-Policy", "cross-origin");
    return res.redirect(302, GEM_EMOJI_URL);
  }
...
// Listening before MongoDB connects lets Render's health-check pass and means
// CORS pre-flight / socket.io handshakes are never refused just because the
// database is still warming up.  Routes that touch the DB will receive a
// normal Mongoose "not connected" error and return a 500 — much better than
// the process dying and the whole service going dark.
server.listen(PORT, "0.0.0.0", () => {
  console.log(`API running on port ${PORT}`);
});

// ── MongoDB with exponential-backoff retry ───────────────────────────────────
// Never call process.exit() on a connection failure.  Mongoose will keep the
// in-flight connection queue and drain it once the connection is (re)established.
let _startupRan = false;

async function connectMongo(attempt = 1) {
  if (!mongoUri) {
    console.error("MONGODB_URI is not set — database features unavailable.");
    return;
  }
  try {
    ...
    setTimeout(() => connectMongo(attempt + 1), delaySec * 1000);
  }
}
