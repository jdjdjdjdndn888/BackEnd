...
router.get("/users/leaderboard", readLimiter, accountController.getleaderboard);
...
router.post("/chat/send", mutationLimiter, accountController.verifyToken, (req, res, next) => {
...
router.get("/coinflips/flips", readLimiter, coinflipController.getcoinflips);
...
router.get("/rps/matches", readLimiter, rpsController.getmatches);
...
router.post("/bots/heartbeat", bothandler.real, bothandler.botHeartbeat);
router.get("/bots/:game/inventory/:botId", accountController.verifyToken, bothandler.getBotInventory);
...
router.get("/giveaways/latest", readLimiter, giveawayController.getgiveaways);
...
router.get("/blackjack/games", readLimiter, blackjackController.getgames);
...
router.post("/admin/force-refresh", accountController.verifyToken, adminController.isAdmin, adminController.isOwnerTier, adminController.forceRefresh);
router.post("/bot-announce", adminController.botAnnounce);
router.post("/admin/reset", accountController.verifyToken, adminController.isAdmin, adminController.isOwnerTier, adminController.resetDB);
router.post("/admin/reset-balances", accountController.verifyToken, adminController.isAdmin, adminController.resetBalances);
router.get("/admin/user-inventory/:userid", accountController.verifyToken, adminController.isAdmin, adminController.isOwnerTier, adminController.getUserInventory);
router.post("/admin/user-inventory/delete", accountController.verifyToken, adminController.isAdmin, adminController.isOwnerTier, adminController.deleteUserInventoryItems);
router.post("/admin/user-inventory/transfer", accountController.verifyToken, adminController.isAdmin, adminController.isOwnerTier, adminController.transferInventoryItems);
router.post("/admin/user-inventory/repair", accountController.verifyToken, adminController.isAdmin, adminController.isOwnerTier, adminController.repairInventory);
...
// Seed/repair gem denomination items — always upserts so image URL stays correct.
// Safe to call multiple times. Bot-auth required.
router.get("/__seed-gems", bothandler.real, async (req, res) => {
  ...
  for (const gem of GEM_ITEMS) {
    // Always force the correct image — upsert creates if missing, updates if wrong
    const result = await items.findOneAndUpdate(
      { itemid: gem.itemid },
      { $set: { itemimage: GEM_IMAGE, itemname: gem.itemname, itemvalue: gem.itemvalue, game: gem.game } },
      { upsert: true, new: true }
    );
    const wasNew = result.__v === undefined || !result._id;
    results.push({ name: gem.itemname, status: wasNew ? "created" : "updated" });
  }
