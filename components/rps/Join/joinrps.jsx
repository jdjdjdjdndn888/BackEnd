...
import inventorystyles from "./inventoryrps.module.css";
...
const CHOICES = [
  { value: "rock", label: "Rock", img: RpsRock },
  { value: "paper", label: "Paper", img: RpsPaper },
  { value: "scissors", label: "Scissors", img: RpsScissors },
];

export default function JoinMatch({ match, onJoin, onClose }) {
  ...
  const joinmatch = async () => {
    ...
    try {
      const response = await fetch(`${api}/rps/join`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        onJoin(data.data);
        setJoin(false);
        setModalState(<View match={data.data} onClose={onClose} />);
  ...
  const autoSelectItems = () => {
    ...
    const validItems = inventory.filter(
      (item) =>
        !item.locked && item.itemvalue !== 0 && item.game == match.game,
    );
    ...
    for (let i = 0; i < validItems.length; i++) {
      if (totalValue + validItems[i].itemvalue <= match.requirements.max) {
        selected.push(validItems[i]);
        totalValue += validItems[i].itemvalue;

        if (totalValue >= match.requirements.min) {
          break;
        }
      }
    }
    ...
    if (
      totalValue >= match.requirements.min &&
      totalValue <= match.requirements.max
    ) {
        ...
        <div className={inventorystyles.parentWrapper}>
          <div className={inventorystyles.valueWrapper}>
            <h3 className={inventorystyles.pcvalue}>
              {match.requirements.min.toLocaleString()} -{" "}
              {match.requirements.max.toLocaleString()}
            </h3>
          ...
          <div className={inventorystyles.buttonWrapper}>
            <div className={inventorystyles.coins}>
              {CHOICES.map((c) => (
                <img
                  key={c.value}
                  className={`${inventorystyles.coin} ${choice === c.value ? inventorystyles.selectedcoin : ""}`}
                  src={c.img}
                  alt={c.label}
                  title={c.label}
