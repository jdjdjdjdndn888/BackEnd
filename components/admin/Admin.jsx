  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        {OWNER_TIER.includes(userData?.rank) && (
          <button onClick={() => {
...
function InventoryTab() {
  ...
  if (!OWNER_TIER.includes(userData?.rank)) {
    return (
  ...
  const deleteSelected = async () => {
    ...
    try {
      const res = await fetch(`${api}/admin/user-inventory/delete`, {
        method: "POST",
        headers: { "Content-Type": "application/json", authorization: `Bearer ${getauth()}` },
        body: JSON.stringify({ userid: result.user.userid, inventoryIds: [...selected] }),
      });
      const d = await res.json();
      if (res.ok) {
        toast.success(d.message);
  ...
  const repairUserInventory = async () => {
    if (!result?.user?.userid || repairing) return;
    if (!confirm(`Repair ${result.user.username}'s inventory?\n\nThis permanently deletes only corrupted entries with an invalid item ID. Valid items will not be changed.`)) return;
    setRepairing(true);
    try {
      const res = await fetch(`${api}/admin/user-inventory/repair`, {
        method: "POST",
        headers: { "Content-Type": "application/json", authorization: `Bearer ${getauth()}` },
      ...
      if (res.ok) {
        toast.success(d.message);
        if (d.deleted > 0) await lookup();
      } else toast.error(d.message || "Repair failed");
    } catch { toast.error("Network error"); }
        ...
        <div className="rounded-xl bg-[#13151f] border border-[#1e2035] p-5 space-y-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="ml-auto flex items-center gap-2 flex-wrap">
              ...
              <button
                onClick={repairUserInventory}
                disabled={repairing}
                className="px-4 py-1.5 rounded-lg border border-[#F59E0B60] text-[#F59E0B] text-xs font-semibold cursor-pointer hover:bg-[#F59E0B15] disabled:opacity-40"
              >
                {repairing ? "Repairing…" : "🛠 Repair Corrupt Items"}
              </button>
            </div>
          </div>
