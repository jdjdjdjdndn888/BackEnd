...
export default function JoinDice({ game, onJoin, onClose }) {
  ...
  const autoSelect = () => {
    if (!inventory.length) return toast.error("No items!");
    const valid = inventory.filter((i) => !i.locked && i.itemvalue > 0 && (game.game === i.game || true))
      .sort((a, b) => b.itemvalue - a.itemvalue);
    let selected = [], tv = 0;
    for (const item of valid) {
      if (tv + item.itemvalue <= game.requirements.max) {
        selected.push(item);
        tv += item.itemvalue;
        if (tv >= game.requirements.min) break;
      }
    }
    if (tv >= game.requirements.min && tv <= game.requirements.max) {
      setSelectedItems(selected);
      setSelectedValue(tv);
    } else {
      toast.error("Could not find matching value.");
    }
  };
  ...
  const joinGame = async () => {
    ...
    setJoining(true);
    try {
      const res = await fetch(`${api}/dice/join`, {
        method: "POST",
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success("Joined! Dice are rolling…");
        onJoin?.(data.data);
        setModalState(<DiceView game={data.data} onClose={onClose} />);
      } else {
        toast.error(data.message);
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={closeModal}>
      ...
      >
        ...
        <div className="p-5 border-b border-[#252839] flex items-center gap-3">
          <h2 className="text-lg font-bold text-white">🎲 Join Dice Game</h2>
          <span className="text-sm text-[#68749C]">vs {game.PlayerOne.username}</span>
        </div>

        <div className="flex items-center gap-2 px-5 py-2 border-b border-[#252839] text-sm">
          <span className="text-[#68749C]">Required value:</span>
          <span className="font-bold text-white">R${formatLargeNumber(game.requirements.min)} – R${formatLargeNumber(game.requirements.max)}</span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 px-5 py-3 border-b border-[#252839]">
          <input
            ...
            className="flex-1 min-w-[160px] rounded-lg border border-[#252839] bg-[#1C1F2E] px-3 py-2 text-sm text-white placeholder-[#42496B] outline-none"
          />
          <Select onValueChange={setSortOrder} value={sortOrder}>
            <SelectTrigger className="h-9 w-36 rounded-lg border border-[#252839] bg-[#1C1F2E] px-3 text-sm text-white">
              <SelectValue />
            </SelectTrigger>
          ...
          <Select onValueChange={setSelectedGame} value={selectedGame}>
            <SelectTrigger className="h-9 w-28 rounded-lg border border-[#252839] bg-[#1C1F2E] px-3 text-sm text-white">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="bg-[#1C1F2D] border border-[#252839] rounded-lg p-1 mt-1">
              {["all", "Sab", "PS99"].map((g) => (
                <SelectItem key={g} className="text-white px-3 py-2 cursor-pointer" value={g}>{g === "all" ? "All" : g}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <button onClick={autoSelect} className="px-3 py-2 rounded-lg text-sm font-semibold text-white border-none cursor-pointer hover:opacity-90" style={{ background: "linear-gradient(135deg,#8B5CF6,#7C3AED)" }}>
            Auto
          </button>
        </div>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-[#252839]">
          <div>
            <p className="text-sm text-[#68749C]">{selectedItems.length} selected</p>
            <p className="text-lg font-bold text-white">R${formatLargeNumber(selectedValue)}</p>
          </div>
          <button
            onClick={joinGame}
            disabled={joining || !selectedItems.length}
          ...
          >
            {joining ? "Joining..." : `Join R$${formatLargeNumber(selectedValue)}`}
          </button>
        </div>
      </div>
    </div>
