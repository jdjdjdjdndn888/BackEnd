...
export default function CreateDice({ onCreate, onClose }) {
  ...
  const createGame = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success("Dice game created!");
        onCreate?.(data.data);
        closeModal();
      } else {
        toast.error(data.message);
    ...
    >
      ...
      >
        ...
        <div className="p-5 border-b border-[#252839] flex items-center gap-3">
          <h2 className="text-lg font-bold text-white">🎲 Create Dice Game</h2>
        </div>
          ...
          {filteredInventory.map((item) => {
            ...
            return (
              <div
                ...
                className={`relative cursor-pointer rounded-xl overflow-hidden border-2 transition-all p-1.5 ${isSelected ? "border-[#8B5CF6] bg-[#8B5CF620]" : "bg-[#1C1F2E]"}`}
