...
export default function SendTradeRequest({ listing }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (res.ok) setInventory((data.data || []).filter((i) => !i.locked));
  ...
  const send = async () => {
    if (!selectedItems.length) return toast.error("Select items to offer!");
    setSending(true);
    try {
      const res = await fetch(`${api}/trades/request`, {
        method: "POST",
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success("Trade request sent! They'll get a notification.");
        closeModal();
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={closeModal}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1e2035]">
          <h2 className="text-white font-bold text-lg">Send Trade Offer</h2>
        ...
        <div className="p-4">
          <div className="rounded-lg border border-[#252839] bg-[#131623] p-3 mb-4">
            <p className="text-[#6B7280] text-xs font-semibold uppercase tracking-wide mb-2">They offer</p>
            <div className="flex flex-wrap gap-2">
              {listing.offeredItems.map((item, i) => (
                <div key={i} className="flex items-center gap-1.5 rounded-lg border border-[#252839] bg-[#0d0f1a] px-2 py-1">
                  {item.itemimage && <img src={item.itemimage} alt={item.itemname} className="w-6 h-6 object-contain" />}
                  <div>
                    <p className="text-white text-[11px] font-medium truncate max-w-[90px]">{item.itemname}</p>
                    <p className="text-[#8B5CF6] text-[10px]">R${item.itemvalue?.toLocaleString()}</p>
                  </div>
                </div>
              ))}
            </div>
            {listing.wantedDescription && (
              <p className="text-[#6B7280] text-xs mt-2">
                <span className="text-[#8B5CF6]">Looking for:</span> {listing.wantedDescription}
              </p>
            )}
          </div>

          <p className="text-[#6B7280] text-xs font-semibold uppercase tracking-wide mb-2">Your offer</p>
          <input
            type="text"
          ...
          <div className="max-h-48 overflow-y-auto grid grid-cols-2 gap-2 mb-3">
            {loading ? (
              ...
              <div className="col-span-2 text-center py-6 text-[#6B7280] text-sm">No items available</div>
            ) : (
              filteredInventory.map((item) => {
                ...
                return (
                  ...
                  >
                    {item.itemimage && <img src={item.itemimage} alt={item.itemname} className="w-8 h-8 object-contain flex-shrink-0" />}
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={send}
              disabled={sending || !selectedItems.length}
