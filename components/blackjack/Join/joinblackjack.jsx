...
export default function JoinBlackjack({ game, onJoin, onClose }) {
  ...
  const autoSelect = () => {
    ...
    const valid = inventory.filter((i) => !i.locked && i.itemvalue > 0)
  ...
  const joinGame = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success("Joined! Dealing cards…");
        onJoin?.(data.data);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={closeModal}>
      ...
      >
        ...
        <div className="p-5 border-b border-[#252839] flex items-center gap-3">
          <h2 className="text-lg font-bold text-white">🃏 Join Blackjack 1v1</h2>
