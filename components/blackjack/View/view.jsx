...
export default function BlackjackView({ game, onClose }) {
  ...
  const actPendingRef = useRef(false);
  ...
  const act = async (action) => {
    if (actPendingRef.current) return;
    actPendingRef.current = true;
    setActing(true);
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => { onClose?.(); setModalState(null); }}>
      <div className="relative w-full max-w-lg rounded-2xl border border-[#252839] shadow-2xl overflow-hidden p-6"
        style={{ backgroundImage: "url(/bg-blackjack.png)", backgroundSize: "cover", backgroundPosition: "center" }}
