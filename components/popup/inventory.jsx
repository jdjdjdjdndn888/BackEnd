...
export default function InventoryModal() {
  ...
  // Gem items: always use the emoji URL regardless of what itemimage says in the DB
  // (the DB may have a stale /item-image?name=... URL that returns a random pet)
  const GEM_IMAGE_URL = "https://cdn.discordapp.com/emojis/1530173247193092146.webp?size=96";
  const GEM_NAME_RE   = /^(?:1m|5m|10m|25m|50m|100m)\s+gems?$/i;
  const resolveImage  = useCallback((name, url) =>
    GEM_NAME_RE.test(String(name)) ? GEM_IMAGE_URL : url
  , []);

  // Detect item variant from name for visual effects on the card border/glow
  const itemVariant = useCallback((name = "") => {
    const n = String(name);
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {filtered.map((item, i) => {
                ...
                const variant = itemVariant(item.itemname);

                // Inline styles for border/glow — Tailwind purges dynamically
                // constructed class strings at build time, so these must be
                // applied via style={} to guarantee they reach the browser.
                const VARIANT_STYLES = {
                  cosmic:  { borderColor: "rgba(139,92,246,0.7)",  boxShadow: "0 0 12px 2px rgba(139,92,246,0.45)", bg: "#12101e", color: "#c4b5fd" },
                  rainbow: { borderColor: "rgba(244,114,182,0.7)", boxShadow: "0 0 12px 2px rgba(244,114,182,0.45)", bg: "#13101a", color: "#f9a8d4" },
                  golden:  { borderColor: "rgba(250,204,21,0.8)",  boxShadow: "0 0 14px 3px rgba(250,204,21,0.50)",  bg: "#14120a", color: "#fde047" },
                  gold:    { borderColor: "rgba(250,204,21,0.8)",  boxShadow: "0 0 14px 3px rgba(250,204,21,0.50)",  bg: "#14120a", color: "#fde047" },
                  shiny:   { borderColor: "rgba(125,211,252,0.7)", boxShadow: "0 0 10px 2px rgba(125,211,252,0.40)", bg: "#0d1219", color: "#7dd3fc" },
                };
                const vs = VARIANT_STYLES[variant];
                const cardStyle = isSelected
                  ? { borderColor: "#8B5CF6", boxShadow: "0 0 0 2px rgba(139,92,246,0.5)" }
                  : vs
                  ? { borderColor: vs.borderColor, boxShadow: vs.boxShadow }
                  : { borderColor: "#252839" };

                const imgSrc = resolveImage(item.itemname, item.itemimage);

                return (
                  <div
                    key={id || i}
                    onClick={() => toggleSelect(id)}
                    className="relative flex flex-col rounded-xl bg-[#1C1F2E] overflow-hidden group cursor-pointer transition-all select-none"
                    style={{
                      border: isSelected
                        ? "2px solid #8B5CF6"
                        : vs
                        ? `2px solid ${vs.borderColor}`
                        : "1px solid #252839",
                      boxShadow: isSelected
                        ? "0 0 0 2px rgba(139,92,246,0.4), inset 0 0 20px rgba(139,92,246,0.15)"
                        : vs
                        ? `inset 0 0 22px ${vs.borderColor.replace(/[\d.]+\)$/, "0.18)")}, 0 0 0 1px ${vs.borderColor}`
                        : "none",
                    }}
                  >
                    {isSelected && (
                      <div className="absolute top-2 right-2 z-40 w-5 h-5 rounded-full bg-[#8B5CF6] flex items-center justify-center shadow">
                    ...
                    <div
                      ...
                      style={{ background: vs ? vs.bg : "#12141f" }}
                    >
                      {imgSrc ? (
                        <>
                          <img
                    ...
                    <div className="px-2 py-2">
                      <p
                        ...
                        style={{ color: vs ? vs.color : "#ffffff" }}
                      >
