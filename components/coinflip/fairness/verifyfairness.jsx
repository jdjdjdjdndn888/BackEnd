...
export default function VerifyFairness({ coinflip, onClose } = {}) {
  const [randomSeed, setRandomSeed] = useState(coinflip?.randomSeed || "");
  const [serverSeed, setServerSeed] = useState(coinflip?.serverSeed || "");
  const [starterValue, setStarterValue] = useState(
    coinflip?.PlayerOne?.items?.length
      ? String(
          coinflip.PlayerOne.items.reduce((acc, i) => acc + i.itemvalue, 0),
        )
      : "",
  );
  const [joinerValue, setJoinerValue] = useState(
    coinflip?.PlayerTwo?.items?.length
      ? String(
          coinflip.PlayerTwo.items.reduce((acc, i) => acc + i.itemvalue, 0),
        )
      : "",
  );
  ...
  const validateFairness = async () => {
    ...
    try {
      ...
      const hashMatches = coinflip?.serverSeedHash
        ? await verifyHash(serverSeed, coinflip.serverSeedHash)
        : null;
      setResult({ normalizedResult, side, hashMatches });
    } catch (error) {
          ...
          <div className={ViewStyles.resultSection}>
            ...
            <div className={ViewStyles.resultText}>
              ...
              {coinflip?.winnercoin && (
                <>
                  <br />
                  Actual Recorded Winner: {coinflip.winnercoin}{" "}
                  {result.side === coinflip.winnercoin ? "✅ Matches" : "❌ Mismatch"}
                </>
              )}
            </div>
          </div>
