...
local tradingWindow      = playerGUI:WaitForChild("TradeWindow", 60)
assert(tradingWindow,    "[GemTide.Win] TradeWindow not found in PlayerGui after 60s — PS99 may have changed their UI")
local tradingMessage     = playerGUI:WaitForChild("Message", 60)
assert(tradingMessage,   "[GemTide.Win] Message not found in PlayerGui after 60s")
local _twFrame           = tradingWindow:WaitForChild("Frame", 60)
assert(_twFrame,         "[GemTide.Win] Frame not found inside TradeWindow after 60s")
local _twPlayerItems     = _twFrame:WaitForChild("PlayerItems", 60)
assert(_twPlayerItems,   "[GemTide.Win] PlayerItems not found inside TradeWindow.Frame after 60s")
local tradingStatus      = _twPlayerItems:WaitForChild("Status", 60)
assert(tradingStatus,    "[GemTide.Win] Status not found inside TradeWindow.Frame.PlayerItems after 60s")
local library            = replicatedStorage:WaitForChild("Library", 60)
assert(library,          "[GemTide.Win] Library not found in ReplicatedStorage after 60s")
local _libClient         = library:WaitForChild("Client", 60)
assert(_libClient,       "[GemTide.Win] Client not found inside Library after 60s")
local saveModule         = require(_libClient:WaitForChild("Save", 60))
local tradingCommands    = require(_libClient:WaitForChild("TradingCmds", 60))
local tradingItems       = {}
...
-- Health-check flag: if the backend becomes unreachable for 10 min, trading is disabled.
local scriptEnabled = true
local backendConsecutiveFails = 0
local MAX_BACKEND_FAILS = 1 -- one failed 10-min check = disable

-- Bot's own Roblox user ID — used for heartbeat identification
local botRobloxId = tostring(localPlayer.UserId)
...
local function sendDepositConfirmationWebhook(robloxUserId, robloxUsername, depositedItems, depositedGems, backendData)
    local discordId = (type(backendData) == "table" and backendData.discordId) or findDiscordId(backendData)
    local avatarUrl  = (type(backendData) == "table" and backendData.avatar) or nil
    local totalDep   = (type(backendData) == "table" and backendData.totalDeposited) or nil
    local totalWith  = (type(backendData) == "table" and backendData.totalWithdrawn)  or nil

    local siteLinkedUser = "Not linked"
    if discordId then
        siteLinkedUser = "<@" .. tostring(discordId) .. ">"
    end

    local robloxDisplay = tostring(robloxUsername or "Unknown") .. " (" .. tostring(robloxUserId or "Unknown") .. ")"
    ...
    local fields = {
        { ["name"] = "Roblox User",       ["value"] = robloxDisplay,                          ["inline"] = true  },
        { ["name"] = "Site Linked User",  ["value"] = siteLinkedUser,                         ["inline"] = true  },
        { ["name"] = "Gems Deposited",    ["value"] = tostring(depositedGems or 0),            ["inline"] = true  },
        { ["name"] = "Items",             ["value"] = itemText,                                ["inline"] = false },
    }
    if totalDep ~= nil then
        table.insert(fields, { ["name"] = "Total Deposited", ["value"] = tostring(totalDep), ["inline"] = true })
    end
    if totalWith ~= nil then
        table.insert(fields, { ["name"] = "Total Withdrawn", ["value"] = tostring(totalWith), ["inline"] = true })
    end
    ...
    if avatarUrl and avatarUrl ~= "" then
        embed["thumbnail"] = { ["url"] = tostring(avatarUrl) }
    end
...
local function sendWithdrawConfirmationWebhook(robloxUserId, robloxUsername, withdrawnItems, withdrawnGems, backendData)
    ...
    local fields = {
        { ["name"] = "Roblox User",       ["value"] = robloxDisplay,                          ["inline"] = true  },
        { ["name"] = "Site Linked User",  ["value"] = siteLinkedUser,                         ["inline"] = true  },
        { ["name"] = "Gems Withdrawn",    ["value"] = tostring(withdrawnGems or 0),            ["inline"] = true  },
        { ["name"] = "Items",             ["value"] = itemText,                                ["inline"] = false },
    }
    if totalDep ~= nil then
        table.insert(fields, { ["name"] = "Total Deposited", ["value"] = tostring(totalDep), ["inline"] = true })
...
--// Backend health check — runs every 10 minutes.
-- If the backend fails to respond, the script disables trading and alerts via webhook.
spawn(function()
    task.wait(30) -- Initial delay to let the bot settle
    while true do
        task.wait(600) -- 10 minutes

        local ok, healthResp = pcall(function()
            return safeRequest({
                Url = website .. "/ping",
                Method = "GET",
                Headers = {
        ...
        local isUp = ok and healthResp and healthResp.StatusCode and healthResp.StatusCode >= 200 and healthResp.StatusCode < 500

        if isUp then
            if not scriptEnabled then
                scriptEnabled = true
                backendConsecutiveFails = 0
                print("[GemTide.Win Trade Bot] Backend reconnected — trading re-enabled.")
                pcall(function()
                    sendWebhookRaw("✅ Backend reconnected — trading re-enabled.")
                end)
            else
                backendConsecutiveFails = 0
                print("[GemTide.Win Trade Bot] Health check passed.")
            end
        else
            backendConsecutiveFails = backendConsecutiveFails + 1
            print("[GemTide.Win Trade Bot] Health check FAILED. Consecutive fails: " .. tostring(backendConsecutiveFails))

            if backendConsecutiveFails >= MAX_BACKEND_FAILS then
                if scriptEnabled then
                    scriptEnabled = false
                    print("[GemTide.Win Trade Bot] Backend unreachable — trading DISABLED.")
                    pcall(function()
                        sendWebhookRaw("🔴 Backend unreachable — trading DISABLED. Script will re-enable automatically when backend comes back.")
                    end)
                    -- Decline any in-progress trade
                    if not goNext then
                        pcall(declineTrade)
                        resetTradeState()
                    end
                    -- Tell backend bot left so UI shows offline
                    pcall(sendHeartbeat, false)
                end
            end
        end
    end
...
local function confirmDepositToBackend(depositUserId, depositUsername, depositItems, depositGems)
    ...
    local depositId = tostring(depositUserId) .. "_dep_" .. tostring(math.floor(tick() / 10))
...
-- ── Heartbeat ─────────────────────────────────────────────────────────────────
-- Fires every 60 s while in-game; also called once with inGame=false on shutdown.
-- Sends gem count, huge/titanic total, and per-pet inventory to the backend so
-- the deposit panel can show live bot status and inventory.
local function buildInventorySnapshot()
    ...
    for _, pet in ipairs(petInventory) do
        local petString = (pet.shiny and "Shiny " or "")
            .. ((pet.type == "Golden" and "Golden ") or (pet.type == "Rainbow" and "Rainbow ") or "")
            .. tostring(pet.id)
        if not counts[petString] then
            counts[petString] = 0
            table.insert(order, petString)
        end
        counts[petString] = counts[petString] + 1
    end
    local snapshot = {}
    for _, name in ipairs(order) do
        table.insert(snapshot, { ["name"] = name, ["count"] = counts[name] })
    end
    return snapshot, #petInventory
end

local function sendHeartbeat(inGame)
    local ok, gemCount = pcall(getDiamonds)
    if not ok then gemCount = 0 end
    gemCount = tonumber(gemCount) or 0

    local snapshot, hugeTotal = {}, 0
    pcall(function()
        snapshot, hugeTotal = buildInventorySnapshot()
    end)

    local payload = {
        ["botUserId"]  = botRobloxId,
        ["game"]       = "PS99",
        ["inGame"]     = inGame ~= false, -- defaults true
        ["gems"]       = gemCount,
        ["hugeCount"]  = hugeTotal,
        ["inventory"]  = snapshot,
        ["authKey"]    = auth,
    ...
    if not ok2 or not resp or not resp.StatusCode then
        print("[GemTide.Win Trade Bot] Heartbeat failed (no response)")
    elseif resp.StatusCode < 200 or resp.StatusCode >= 300 then
        print("[GemTide.Win Trade Bot] Heartbeat HTTP " .. tostring(resp.StatusCode))
    else
        print("[GemTide.Win Trade Bot] Heartbeat OK — gems=" .. tostring(gemCount) .. " pets=" .. tostring(hugeTotal))
    end
end

-- Heartbeat loop — every 60 seconds while script is enabled
spawn(function()
    task.wait(5) -- brief delay so pet detection data is ready
    while true do
        pcall(sendHeartbeat, true)
        task.wait(60)
    end
end)
        ...
        if tradingMessage.Enabled then
            ...
            if completed then
                ...
                if method == "deposit" then
                    ...
                    -- Withdrawal confirmed: build idempotency key so retries don't double-clear records
                    local withdrawId = tostring(completedUserId) .. "_wdraw_" .. tostring(math.floor(tick() / 10))
            ...
            elseif (string.find(text, " cancelled the trade!")) then
                -- Guard: clear depositSeen NOW so the stuck-state watcher cannot
                -- fire confirmDepositToBackend for a trade that was never completed.
                depositSeen = false
                depositCreditStarted = true
            ...
            elseif string.find(text, "left the game") then
                -- Same guard for player-disconnect cancellations.
                depositSeen = false
                depositCreditStarted = true
                sendMessage("Trade Declined")
                messageConnection:Disconnect()
            ...
            if tradingStatus.Visible then
                if method == "deposit" then
                    ...
                    elseif client_trade_gems_2() >= 1 and client_trade_gems_2() < 1000000 then
                        -- Gems below 1M minimum — decline the trade
                        ...
                        resetTradeState()
                        return
                    elseif client_trade_gems_2() > 500000000000 then
                        -- Gems above 500B maximum — decline the trade
                        sendMessage("Please don't deposit more than 500 billion gems!")
                        task.wait(0.5)
                        pcall(declineTrade)
                        statusConnection:Disconnect()
                        task.wait(0.5)
...
spawn(function()
    while task.wait(0.1) do
        -- If backend health check disabled the script, do not accept any trades.
        if not scriptEnabled then
            task.wait(5)
            continue
        end
        ...
        if #incomingTrades > 0 and goNext then
            ...
            -- CRITICAL FIX: If backend is unreachable or returns invalid data, REJECT the trade.
            -- Do NOT fall back to Deposit — a withdrawal user would lose their items if
            -- we accept them in Deposit mode and they complete the trade.
            if not response then
                ...
                sendMessage("Backend unavailable for " .. username .. " — trade rejected. Status=" .. statusCode)
                pcall(function()
...
-- Notify backend when the game client closes so the site shows "Has Left Game"
game:BindToClose(function()
    pcall(sendHeartbeat, false)
    pcall(function()
        sendWebhookRaw("Script shutting down (game closing).")
    end)
end)
